...
class MPNNModel(Module):
    def __init__(self, num_layers=1, emb_dim=64, in_dim=11, edge_dim=4, out_dim=1):
        ...
        """
        super().__init__()
        self.coord_dim =3 #added for Task 1.1
    ...
    def forward(self, data):
        ...
        """
        features_with_pos = torch.cat(( data.x, data.pos),1)
        # print(features_with_pos.size())
...
class CoordMPNNModel(MPNNModel):
    def __init__(self, num_layers=1, emb_dim=64, in_dim=11, edge_dim=4, out_dim=1):
        ...
        self.coord_dim = 3
        self.lin_in = Linear(in_dim + self.coord_dim, emb_dim)
        # Stack of MPNN layers
        self.convs = torch.nn.ModuleList()
        for layer in range(num_layers):
            self.convs.append(MPNNLayer(emb_dim, edge_dim, aggr='add'))
